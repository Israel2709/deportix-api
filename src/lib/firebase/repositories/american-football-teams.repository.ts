...
export async function updateAmericanFootballTeam(
  ...
  patch: Partial<AmericanFootballTeamItem>,
): Promise<RawDoc> {
  const existing = await resolveDoc(COLLECTION, idOrExternalId);
  if (!existing) throw notFound('Team not found.');

  const payload = {
    ...(existing.data.api_sports_payload as AmericanFootballTeamItem | undefined),
    ...patch,
    id: patch.id ?? (existing.data.api_sports_payload as AmericanFootballTeamItem | undefined)?.id ?? existing.data.external_id,
  };
